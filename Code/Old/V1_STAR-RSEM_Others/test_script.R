...
load("/external/rprshnas01/kcni/ychen/git/PSQ_Pipeline/convertedgtf_reference.count.Rdata") #loading sequence reference
...
dir.create(paste("/external/rprshnas01/netdata_kcni/stlab/File_transfer/RefSeqTest/ForAIBS_InEx", "/InExCounts/", sep = "")) #create directory for output

# list of bam files in given directory
folder_files <- list.files(path = "/external/rprshnas01/netdata_kcni/stlab/File_transfer/RefSeqTest/ForAIBS_InEx/", pattern = "\\.bam$")

for (i in folder_files) {
  
  fn = paste0("/external/rprshnas01/netdata_kcni/stlab/File_transfer/RefSeqTest/ForAIBS_InEx", "/", i)
  ...
  df<- data.frame(gene_id=anno$gene_id,gene_symbol=names(exon.list),length=exonic.gene.sizes,intron_count=intron.count,exon_count=exon.count,fpkm=fpkms)
  
  # writing sample-level dataframe (counts of exons, introns, fpkm, etc.) as csv file
  write.csv(df, file = paste("/external/rprshnas01/netdata_kcni/stlab/File_transfer/RefSeqTest/ForAIBS_InEx", "/InExCounts/", tools::file_path_sans_ext(i), "_count.csv", sep = ""))
