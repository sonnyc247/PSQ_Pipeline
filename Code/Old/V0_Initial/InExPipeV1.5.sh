...
if [ "$pairend" = false ] ; then
  ...
  for file in `find $datadir -maxdepth 1 -type f | sort`; do
    ...
    echo STAR --runThreadN 12 --genomeDir /external/rprshnas01/kcni/ychen/References/Ensembl/StarRefMouse/ --sjdbGTFfile /external/rprshnas01/kcni/ychen/References/Ensembl/Mus_musculus.GRCm38.98.gtf --sjdbOverhang 100 --readFilesIn $file --quantMode TranscriptomeSAM --outSAMtype BAM SortedByCoordinate --outFileNamePrefix "$res_fold$prefix/$prefix"  >> STARParaScript$tempi.sh
    echo cp "$res_fold$prefix/$prefix"Aligned.sortedByCoord.out.bam "${res_fold}coord_bams/" >> STARParaScript$tempi.sh
    echo cp "$res_fold$prefix/$prefix"Aligned.toTranscriptome.out.bam "${res_fold}trans_bams/" >> STARParaScript$tempi.sh
    echo cp "$res_fold$prefix/$prefix"Log.final.out "${res_fold}star_logs/" >> STARParaScript$tempi.sh
    echo module load RSEM >> STARParaScript$tempi.sh
    echo rsem-calculate-expression --bam --no-bam-output -p 12 --forward-prob 0 "$res_fold$prefix/$prefix"Aligned.toTranscriptome.out.bam /external/rprshnas01/kcni/ychen/References/Ensembl/RSEMRefMouse/ref "$res_fold$prefix/$prefix" >> STARParaScript$tempi.sh
    echo cp "$res_fold$prefix/$prefix".genes.results "${datadir}RSEM_results/" >> STARParaScript$tempi.sh
  ...
  for file in "${forarr[@]}"; do
    ...
    echo module load STAR >> STARParaScript$tempi.sh
    echo mkdir $res_fold$prefix >> STARParaScript$tempi.sh
    echo STAR --runThreadN 12 --genomeDir /external/rprshnas01/kcni/ychen/References/Ensembl/StarRefMouse/ --sjdbGTFfile /external/rprshnas01/kcni/ychen/References/Ensembl/Mus_musculus.GRCm38.98.gtf --sjdbOverhang 100 --readFilesIn $file $file2 --quantMode TranscriptomeSAM --outSAMtype BAM SortedByCoordinate --outFileNamePrefix "$res_fold$prefix/$prefix"  >> STARParaScript$tempi.sh
    echo cp "$res_fold$prefix/$prefix"Aligned.sortedByCoord.out.bam "${res_fold}coord_bams/" >> STARParaScript$tempi.sh
    echo cp "$res_fold$prefix/$prefix"Aligned.toTranscriptome.out.bam "${res_fold}trans_bams/" >> STARParaScript$tempi.sh
    echo cp "$res_fold$prefix/$prefix"Log.final.out "${res_fold}star_logs/" >> STARParaScript$tempi.sh
    echo module load RSEM >> STARParaScript$tempi.sh
    echo rsem-calculate-expression --bam --no-bam-output -p 12 --paired-end --forward-prob 0 "$res_fold$prefix/$prefix"Aligned.toTranscriptome.out.bam /external/rprshnas01/kcni/ychen/References/Ensembl/RSEMRefMouse/ref "$res_fold$prefix/$prefix" >> STARParaScript$tempi.sh
    echo cp "$res_fold$prefix/$prefix".genes.results "${datadir}RSEM_results/" >> STARParaScript$tempi.sh
