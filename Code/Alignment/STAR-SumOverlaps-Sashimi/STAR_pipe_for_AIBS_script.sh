...
if [ "$pairend" = false ] ; then
  ...
  for file in `find $datadir -maxdepth 1 -type f | sort`; do
    ...
    echo STAR --genomeDir $genomePath --runThreadN 6 --sjdbGTFfile $annotationPath --readFilesIn $file --outFileNamePrefix "$res_fold$prefix/$prefix" --quantMode GeneCounts --outSAMtype BAM SortedByCoordinate --readFilesCommand zcat >> "${outputdir}/STAR_scripts/STARParaScript${tempi}.sh"
    echo mv "$res_fold$prefix/$prefix"Aligned.sortedByCoord.out.bam "${res_fold}coord_bams/" >> "${outputdir}/STAR_scripts/STARParaScript${tempi}.sh"
  ...
  for file in "${forarr[@]}"; do
    ...
    echo module load STAR >> "${outputdir}/STAR_scripts/STARParaScript${tempi}.sh"
    echo mkdir $res_fold$prefix >> "${outputdir}/STAR_scripts/STARParaScript${tempi}.sh"
    echo STAR --genomeDir $genomePath --runThreadN 6 --sjdbGTFfile $annotationPath --readFilesIn $file $file2 --outFileNamePrefix "$res_fold$prefix/$prefix" --quantMode GeneCounts --outSAMtype BAM SortedByCoordinate --readFilesCommand zcat >> "${outputdir}/STAR_scripts/STARParaScript${tempi}.sh"
    echo mv "$res_fold$prefix/$prefix"Aligned.sortedByCoord.out.bam "${res_fold}coord_bams/" >> "${outputdir}/STAR_scripts/STARParaScript${tempi}.sh"
    echo mv "$res_fold$prefix/$prefix"Log.final.out "${res_fold}star_logs/Init_QCMetrics/" >> "${outputdir}/STAR_scripts/STARParaScript${tempi}.sh"
    echo mv "$res_fold$prefix/$prefix"Log.out "${res_fold}star_logs/Init_Runmsgs/" >> "${outputdir}/STAR_scripts/STARParaScript${tempi}.sh"
